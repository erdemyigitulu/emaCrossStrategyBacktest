import json
import numpy as np
from calculations import percentageIncrease , calculateAvaregePrice , moneyProfitLossFunc
from config import fixedMonths, backtestConfigs , years , months
from utils import convertCsvToParquet , extractSignals , extractPurchasedPoints , improvingPurchasedPoints
...
def backTestofMachine () :
    for year in years :
        for month in months :
            datas15m , resultscsv , csv1s , parquet1s = fixedMonths(month,year)
            signals = extractSignals(datas15m)
            data1s = convertCsvToParquet(csv1s,parquet1s) 
            resultDatas = []
            totalpnL = profit = loss = profitsCount = lossesCount = entryStopsCount = 0
            for index , signal in enumerate(signals[1:]) :
                try :
                    nextSignal = signals[(index + 2)][1]
                except :
                    break
                purchasePoints , purchasedPoints , startIndex , startProcessTimeStamp , firstValue = extractPurchasedPoints(signal , data1s)
                seperateMoney , profitLoss , portion , messages , _ = backtestConfigs()
                a = b = c = aa = bb = True
                machine = Machine()
                for past in data1s[startIndex:]:
                    purchasedPoints , currentValue , currentTimeStamp = improvingPurchasedPoints( signal , purchasedPoints , purchasePoints , past , seperateMoney)
